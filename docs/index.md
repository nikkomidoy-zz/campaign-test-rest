...
```
heroku create campaign-test-prod --remote prod && \
    heroku addons:create newrelic:wayne --app campaign-test-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app campaign-test-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="campaign-test-prod" \
        --app campaign-test-prod
```

Creating the qa sever:

```
heroku create `campaign-test-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="campaign-test-qa" \
```
